...
class Helpers {
    ...
    public static function getRegionsByUser($user_id,$is_get_parent = 1){

        if($is_get_parent == 1 && Auth::user()->role_id == 7) $user_id = Auth::user()->parent_user_id;
    ...
    public static function getRegionsByParent($region_id,$role_id = 0){
        if($role_id == 0) $role_id = Auth::user()->role_id;

        $region = Region::where('region_id',$region_id)->select('parent_id')->first();

        if($role_id == 9 && $region != null && $region->parent_id != null) $region_id = $region->parent_id;

        $ids[] = $region_id;

        $child_regions = Region::where('parent_id',$region_id)->select('region_id')->get();
        foreach($child_regions as $child_item) {
            $ids[] = $child_item['region_id'];
        }
    ...
    public static function getRegionsByUserAPI($user_id,$is_get_parent = 1){
        if($is_get_parent == 1) {
            $user = Users::where('user_id',$user_id)->first();
            if($user->role_id == 7) $user_id = $user->parent_user_id;
        }
    ...
    public static function getRoles($role_id,$user_id = 0){
        $roles = Role::orderBy('role_id','asc');

        if($role_id == 8){
            $roles->where('role_id',8)->orWhere('role_id',9);
        }
        elseif($role_id == 9){
            if($user_id == 0)
            ...
            $region = Region::where('region_id',$region_id)->select('parent_id','region_child_count')->first();

            if($region->region_child_count == 0){
                $roles->where(function($query) {
                    $query->where('role_id',9)->orWhere('role_id',11);
                });
            }
            else {
                $roles->where(function($query) {
                    $query->where('role_id',9)->orWhere('role_id',10);
                });
            }
        }
        elseif($role_id == 10){
            $roles->where(function($query) {
                $query->where('role_id',10)->orWhere('role_id',11);
            });
        }
        elseif($role_id == 11){
            $roles->where(function($query) {
                $query->where('role_id',11)->orWhere('role_id',12)->orWhere('role_id',3);
            });
        }
    ...
    public static function getRegions($role_id,$user_id = 0){
        $regions = Region::orderBy('parent_id','asc')->orderBy('region_name_ru','asc');
        ...
        if($role_id == 9){
            $parent_region_id = 0;
            $region_parent = Region::where('region_id',$region_id)->select('parent_id')->first();

            if($region_parent != null && $region_parent->parent_id != null) $parent_region_id = $region_parent->parent_id;

            $regions->where(function($query) use ($region_id,$parent_region_id) {
                $query->where('parent_id',$parent_region_id)->orWhere('region_id',$region_id)->orWhere('parent_id',$region_id);
            });
        }
        elseif($role_id == 10){
            $regions->where(function($query) use ($region_id) {
    ...
    public static function getSchools($role_id){
        $schools = School::orderBy('school_name_ru','asc');
        $region_id = Auth::user()->region_id;

        if($role_id == 10 || $role_id == 9){
            $schools->where(function($query) use ($region_id) {
                $query->where('school.region_id',$region_id);
            });
        }
        else {
            $schools->where(function($query) use ($region_id) {
    ...
    public static function checkHasChildRegion($region_id){
        $region = Region::where('region_id',$region_id)->where('region_child_count',0)->whereNull('parent_id')->count();
        return $region;
    }
} 